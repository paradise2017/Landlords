...
//QMap<Card, CardPanel*> m_cardMap 中添加卡片和卡片图案
...
void GamePanel::startDispatchCard()
{
    ...
    {
        it.value()->setSeclected(false); //窗口选择状态
        it.value()->setFrontSide(true);  //显示窗口正面
    ...
    // 重置玩家的窗口上下文信息
    int index = m_playerList.indexOf(m_gameCtl->getUserPlayer());               //用户玩家对应的索引值
    for(int i=0; i<m_playerList.size(); ++i)
    {
        //出牌区域；头像；已存在的牌不需要改正，
        m_contextMap[m_playerList.at(i)].lastCards.clear();                     //最后打出的牌
        m_contextMap[m_playerList.at(i)].info->hide();                          //玩家提示信息
        m_contextMap[m_playerList.at(i)].roleImg->hide();                       //头像位置不变，头像隐藏
        m_contextMap[m_playerList.at(i)].isFrontSide = i==index ? true : false; //机器人背面，当前玩家正面
    }
    // 重置所有玩家的卡牌数据；除了获胜的玩家，其他玩家手里的牌清空
...
void GamePanel::updatePlayerCards(Player *player)
{
    ...
    CardList list = cards.toCardList();                       //默认降序排列，当前玩家的牌

    m_cardsRect = QRect();
    m_userCards.clear();
    // 取出展示扑克牌的区域
    int cardSpace = 20;                                        //显示牌区域的间隙20

    QRect cardsRect = m_contextMap[player].cardRect;           //当前玩家牌的显示区域
    for(int i=0; i<list.size(); ++i)
    {
        CardPanel* panel = m_cardMap[list.at(i)];              //当前牌的窗口
        panel->show();                                         //当前牌显示
        panel->raise();                                        //当前提升
        panel->setFrontSide(m_contextMap[player].isFrontSide); //显示正面还是背面
...
void GamePanel::onDispatchCard()
{
    ...
    {
        // 给玩家发一张牌
        Card card = m_gameCtl->takeOneCard();           //从总的牌库拿一张牌；
        curPlayer->storeDispatchCard(card);             //当前牌
        Cards cs(card);
//        disposeCard(curPlayer, cs);
        ...
        // 判断牌是否发完了
        if(m_gameCtl->getSurplusCards().cardCount() == 3)       //游戏控制类中剩下的牌 cards
        {
            // 终止定时器
            m_timer->stop();
            // 切换游戏状态
...
void GamePanel::onPlayerStatusChanged(Player *player, GameControl::PlayerStatus status)
{
    ...
    case GameControl::ThinkingForCallLord:
        if(player == m_gameCtl->getUserPlayer())          //机器人不需要，用户玩家需要按钮组
        {
